...
/* === 🟢 НАЧАЛО ИЗМЕНЕНИЙ (ВЫСОТА ПЛАНШЕТА v5.5) === */
.graph-area-wrapper {
  overflow-x: hidden;
  overflow-y: hidden;
  scrollbar-width: none;
...
/* === 🟢 НАЧАЛО ИЗМЕНЕНИЙ (ШРИФТЫ/ВЫСОТА ДЛЯ ПЛАНШЕТА v5.5) === */
...
@media (max-height: 900px) {
  ...
  .timeline-grid-wrapper, .nav-panel-wrapper {
    height: 280px; 
  }

  .header-resizer {
    height: 8px; /* Делаем ресайзер тоньше */
  }
  .divider-wrapper {
    height: 8px; /* И нижний ресайзер/скроллбар */
  }
  .summaries-container {
    height: 75px; /* Еще уменьшаем блок итогов */
    padding: 0.5rem 1rem;
  }
  .import-export-btn {
    width: 28px;
