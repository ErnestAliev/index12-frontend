...
/* === 🟢 НАЧАЛО ИЗМЕНЕНИЙ (ВЫСОТА ПЛАНШЕТА v5.3) === */
.graph-area-wrapper {
  overflow-x: hidden;
  overflow-y: hidden;
  scrollbar-width: none;
...
.summaries-container {
  ...
  height: 90px; /* Высота для итогов (можно настроить) */
...
/* === 🟢 НАЧАЛО ИЗМЕНЕНИЙ (ШРИФТЫ ДЛЯ ПЛАНШЕТА v5.3) === */
/* * Мы используем 'max-height' вместо 'max-width'.
 * Это надежнее определяет "планшетный" (невысокий) режим.
*/
@media (max-height: 900px) {
  .header-resizer {
    height: 8px; /* 🔴 ИСПРАВЛЕНИЕ v5.3: Делаем ресайзер тоньше */
  }
  .divider-wrapper {
    height: 8px; /* 🔴 ИСПРАВЛЕНИЕ v5.3: И нижний ресайзер/скроллбар */
  }
  .summaries-container {
    height: 75px; /* 🔴 ИСПРАВЛЕНИЕ v5.3: Еще уменьшаем блок итогов */
    padding: 0.5rem 1rem;
  }
  .import-export-btn {
    width: 28px;
...
/* 🔴 НОВОЕ (v5.3): Адаптация под ширину (960px - 1440px) */
