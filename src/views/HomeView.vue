...
const sendAiMessage = async (forcedMsg = null, opts = {}) => {
  ...
  try {
    ...
    // Передаем активный период фильтра, чтобы AI считал только его (например, январь)
    const periodFilter = mainStore?.periodFilter ? { ...mainStore.periodFilter } : null;

    // Таймлайн дневных итогов (как в графике) — расширяем до 90 дней для анализа/прогноза
    const buildTimelinePayload = () => {
      ...
      let start = null;
      let end = null;
      const pf = mainStore?.periodFilter;
      if (pf && pf.mode === 'custom' && pf.customStart && pf.customEnd) {
        start = new Date(pf.customStart);
        end = new Date(pf.customEnd);
      } else if (projectionStore?.projection?.rangeStartDate && projectionStore?.projection?.rangeEndDate) {
        start = new Date(projectionStore.projection.rangeStartDate);
        end = new Date(projectionStore.projection.rangeEndDate);
      }
      ...
      if (start > ninetyDaysAgo) start = ninetyDaysAgo; // расширяем назад до 90дн

      start.setHours(0, 0, 0, 0);
      end.setHours(23, 59, 59, 999);

      const out = [];
      timelineMap.forEach((val) => {
        if (!val?.date) return;
    ...
    <div class="home-body" :style="{ '--timeline-height': timelineHeightPx + 'px', '--divider-height': DIVIDER_H + 'px' }">
      ...
      <aside class="home-right-panel">
        ...
        <div class="right-panel-top">
          ...
          <button v-if="!mainStore.isTimelineOnly" class="icon-btn import-export-btn" @click="showImportModal = true" data-tooltip="Экспорт операций">
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
