...
/**
 * * --- МЕТКА ВЕРСИИ: v26.11.17 - HIDE REFUND ---
 * * ВЕРСИЯ: 26.11.17 - Скрытие "Возврат" в списке категорий
 * * ДАТА: 2025-11-26
 *
 * ЧТО ИЗМЕНЕНО:
 * 1. (LOGIC) В фильтре onMounted добавлена проверка на имя "возврат".
 */
...
onMounted(() => {
  ...
  rawItems = rawItems.filter(item => {
      ...
      // Скрываем системные категории
      if (isCategoryEditor) {
          if (name === 'реализация') return false;
          if (name === 'остаток долга') return false;
          if (name === 'возврат') return false; // 🟢 FIX: Скрываем "Возврат"
      }
      return true;
  });
