...
/**
 * * --- МЕТКА ВЕРСИИ: v9.0-step12-STYLE-FIX ---
 * * ВЕРСИЯ: 9.0 - ФИНАЛЬНАЯ ВЕРСИЯ.
 * ДАТА: 2025-11-17
 *
 * ЧТО ИЗМЕЕНО:
 * 1. (BUILD-FIX) Исправлен дубликат :key в <template> (v-if="isContractorEditor")
 * 2. (STYLE-FIX) Исправлен стиль .edit-account-select,
 * чтобы он корректно отображал <select multiple> как
 * аккуратный "list box", а не пытался "закрыть" его.
 */
...
/* 🔴🔴🔴 
  STYLE FIX (v12): 
  <select multiple> - это ВСЕГДА "list box".
  Нельзя его "закрыть". Этот CSS делает его аккуратным.
*/
.edit-account-select {
  flex-shrink: 0;
  width: 310px;
  
  /* ВОЗВРАЩАЕМ стили для "list box" */
  height: 100px;         
  padding: 10px;        
  overflow-y: auto;     
  
  /* УБИРАЕМ стили от "закрытого" селекта (они тут не нужны) */
