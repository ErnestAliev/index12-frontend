...
/**
 * * --- МЕТКА ВЕРСИИ: v18.2 - OPTION FORMAT FIX ---
 * * ВЕРСИЯ: 18.2 - Форматирование списка счетов (удалено тире, добавлены отступы)
 * * ДАТА: 2025-11-21
 *
 * ЧТО ИЗМЕНЕНО:
 * 1. (UI) Удален разделитель "&mdash;" (тире).
 * 2. (UI) Добавлены неразрывные пробелы для визуального разделения названия и баланса.
 * 3. (UI) Формат: "Название      100 000 ₸".
 */
    ...
    <div class="popup-content" :class="popupTheme">
      ...
      <template v-if="props.type !== 'transfer' && !showCreateOwnerModal">
        ...
        <select v-if="!isCreatingAccount" v-model="selectedAccountId" @change="e => e.target.value === '--CREATE_NEW--' ? showAccountInput() : onAccountSelected(e.target.value)" class="form-select">
          ...
          <!-- 🟢 ОБНОВЛЕННЫЙ ФОРМАТ: Удалено тире, добавлены пробелы -->
