...
/* === 🟢 НАЧАЛО ИЗМЕНЕНИЙ (ШРИФТЫ ДЛЯ ПЛАНШЕТА v1.3) === */
@media (max-height: 900px) {
  .hour-cell {
    padding: 2px 4px; /* 🔴 ИСПРАВЛЕНИЕ v1.3: Уменьшаем боковые отступы ячейки */
    height: 28px; /* 🔴 ИСПРАВЛЕНИЕ v1.3: Делаем ячейку еще ниже */
  }
  .operation-chip {
    font-size: 0.7em; /* 🔴 ИСПРАВЛЕНИЕ v1.3: Агрессивное уменьшение шрифта */
    padding: 3px 6px; 
  }
  .op-amount, .op-title {
    margin-right: 4px; 
...
/* 🔴 НОВОЕ (v1.2): Адаптация под ширину (960px - 1440px) */
@media (max-width: 1440px) {
  .hour-cell {
    padding: 4px 6px;
  }
  .operation-chip {
    font-size: 0.75em; /* 🔴 Уменьшаем шрифт чипа */
    padding: 3px 6px;
  }
}
/* === 🟢 КОНЕЦ ИЗМЕНЕНИЙ === */
