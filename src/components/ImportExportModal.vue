<!--
 * * --- МЕТКА ВЕРСИИ: v10.11-TODAY-FIX ---
 * * ВЕРСИЯ: 10.11 - Исправление ошибки,
 * когда операции "сегодня"
 * попадали в "будущее".
 * ДАТА: 2025-11-18
 *
 * ЧТО ИЗМЕНЕНО:
 * 1. (FIX) `prepareExportData` теперь
 * обнуляет время (setHours(0,0,0,0))
 * у `opDate` перед сравнением
 * с `today.getTime()`.
 * 2. (FIX) Это гарантирует, что операции
 * СЕГОДНЯ попадают в `pastOps`
 * (Файлы 1 и 2), а не в `futureOps`.
 -->
<template>
